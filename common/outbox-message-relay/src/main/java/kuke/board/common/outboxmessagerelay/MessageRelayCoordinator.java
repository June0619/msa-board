...
public class MessageRelayCoordinator {
	...
	private final int PING_INTERVALS_SECONDS = 3;
	...
	@Scheduled(fixedDelay = PING_INTERVALS_SECONDS, timeUnit = TimeUnit.SECONDS)
	public void ping() {
		redisTemplate.executePipelined((RedisCallback<?>) action -> {
			...
			conn.zRemRangeByScore(
					...
					Instant.now().minusSeconds(PING_INTERVALS_SECONDS * PING_FAILURE_THRESHOLD).toEpochMilli()
			);
			return null;
		});
	}
